...
import com.xiaohao.kf.config.KafkaProperties;
...
class XinghaokfApplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        XinghaokfApplicationKafkaResource kafkaResource = new XinghaokfApplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/xinghaokf-application-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/xinghaokf-application-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
